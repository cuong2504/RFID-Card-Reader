...
extern void GPIO_Input_Init_Reg(GPIO_TypeDef* gpioPort, 
																uint8_t portLevel,  
																uint8_t gpioPin,
																uint32_t config,  
																bool pullupEn);
...
extern void GPIO_Output_Init(GPIO_TypeDef* gpioPort,
														 uint8_t portLevel,
														 uint32_t mode,
														 uint32_t config);

//Writes a logic 'high' or 'low' to a GPIO pin
extern void GPIO_Output_Write(GPIO_TypeDef* gpioPort,
															uint8_t gpioPin,
															bool gpioPinLogic);

extern void GPIO_Output_Clear_Reg(GPIO_TypeDef* gpioPort,
																	uint32_t gpioPins);
